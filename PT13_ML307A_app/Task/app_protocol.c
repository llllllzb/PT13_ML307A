...
//    DestBuf[pdu_len++ ] = protocolInfo.runCnt & 0xff;
	DestBuf[pdu_len++] = (dynamicParam.runningtime >> 8) & 0xff;
	DestBuf[pdu_len++] = dynamicParam.runningtime & 0xff;
	DestBuf[pdu_len++] = (dynamicParam.step >> 8) & 0xff;
	DestBuf[pdu_len++] = dynamicParam.step & 0xff;
    ret = createProtocolTail(DestBuf, pdu_len,  Serial);
    if (ret < 0)
    {
        return -1;
